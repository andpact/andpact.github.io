...
해당 서비스 계정을 구성원으로 추가하고 아래 두 역할을 부여해야 합니다.

- **Cloud Datastore 소유자**: 이 역할이 없으면 Cloud Functions에서 Firestore에 데이터를 쓸 수 없습니다.
- **서비스 계정 토큰 생성자**: 이 역할이 없으면 Cloud Functions에서 서비스 계정을 통해 Google Play Developer API를 호출할 때 인증 토큰을 발급받을 수 없습니다.
...
권한은 **계정 권한**과 **앱 권한** 두 곳에 각각 설정해야 합니다.
아래 항목을 모두 체크합니다.
...
한 가지 더 주의할 점이 있습니다.
서비스 계정 설정 이전에 이미 만들어 둔 상품이 있다면,
권한을 모두 부여했더라도 해당 상품 조회 시 **401 권한 부족 오류**가 발생할 수 있습니다.
이 경우 Play Console에서 해당 상품을 **비활성화 후 다시 활성화**하면 오류를 방지할 수 있습니다.
