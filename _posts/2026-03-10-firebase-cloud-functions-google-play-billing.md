...
## 6. Cloud Functions에 최초 구매 검증 함수 배포하기
...
4. Google Cloud에서 Google Play Developer API를 활성화한다.
5. 앱이 `purchaseToken`, `productId`, `packageName`, `productType`를 서버로 보내도록 준비한다.
6. Cloud Functions에 최초 구매 검증 함수를 배포한다.
7. 검증 성공 후에만 Firestore와 사용자 권한 상태를 반영한다.
